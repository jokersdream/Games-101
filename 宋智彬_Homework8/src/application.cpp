...
void Application::init() {
  ...
  // Create two ropes 
  ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16, config.mass,       // origin num_nodes = 3; changed to 16
                       config.ks, {0});
  ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16, config.mass,      // origin num_nodes = 3; changed to 16
                        config.ks, {0});
}
